...
import com.unipampa.padel.DTO.ChaveDTO;
...
public class ChaveParser {
	...
	public static String cadastroChave(Chave chave) throws IOException {
		return RestUtil.persisteDados(url_post, convertChaveParaJson(convertChaveParaChaveDTO(chave)));
	}

	public static ChaveDTO convertChaveParaChaveDTO(Chave chave) {

		ChaveDTO chaveDTO = new ChaveDTO();
		chaveDTO.setNome(chave.getNome());
		
		ArrayList<Integer> duplasIds = new ArrayList<>();
		for (Dupla d : chave.getDuplaList()) {
			duplasIds.add(d.getId());
		}
		chaveDTO.setDuplaList(duplasIds);
		
		ArrayList<Integer> etapasIds = new ArrayList<>();
		for (Etapa e : chave.getEtapaList()) {
			etapasIds.add(e.getId());
		}
		chaveDTO.setEtapaList(etapasIds);
		
		return chaveDTO;
	}

	private static String convertChaveParaJson(ChaveDTO chaveDTO) {
